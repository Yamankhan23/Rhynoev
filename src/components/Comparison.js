      ...
      <div className="vehicle-comparison">
        {['SE03 Lite', 'SE03', 'SE03 Max'].map((model, index) => (
          <div className="comparison-card" key={index}>
            <img
              src={`path/to/${model.replace(" ", "").toLowerCase()}.jpg`}
              alt={model}
              className="vehicle-image"
            />
            <h2>{model}</h2>
            <div className="color-options">
              <button className="color-btn color-red" />
              <button className="color-btn color-blue" />
              <button className="color-btn color-black" />
            </div>
            <Link to="/pre-book" className="buy-now-btn">Buy Now</Link> {/* Updated to Link */}
          </div>
        ))}
      </div>
